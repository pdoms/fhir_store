...
use crate::data::datatype::{get_expects, get_expected};
            ...
            if let Ok(num) = self.read_u16() {
                ...
                if let Ok(id) = StoreId::try_from(num) {
                    ...
                    if id.is_primitive() {
                        ...
                        if let Some(exp) = get_expected(id as u16) {
                            if exp.is_primitive() {
                                self.key = Some(id);
                                self.read_unit(tree);
                            } else {
...
mod test {
    ...
    #[test]
    fn tree_parser() {
        let mut data = vec![0, 140, 1, 1, 0, 7, 0, 14, 80, 97, 116, 105, 101, 110, 116, 0, 2, 1, 2, 0, 4, 0, 8, 97, 110, 105, 100, 0, 2, 1, 5, 0, 100, 1, 5, 0, 62, 1, 9, 0, 8, 0, 14, 111, 102, 102, 105, 99, 105, 97, 108, 0, 2, 1, 10, 0, 8, 0, 14, 67, 104, 97, 108, 109, 101, 114, 115, 0, 2, 1, 11, 0, 26, 1, 11, 0, 20, 0, 35, 0, 5, 0, 14, 80, 101, 116, 101, 114, 0, 5, 0, 14, 74, 97, 109, 101, 115, 0, 2, 0, 43, 0, 32, 1, 9, 0, 5, 0, 14, 117, 115, 117, 97, 108, 0, 2, 1, 11, 0, 15, 1, 11, 0, 9, 0, 35, 0, 3, 0, 14, 74, 105, 109, 0, 2, 0, 43, 0, 2, 1, 3, 0, 1, 0, 2, 1];
